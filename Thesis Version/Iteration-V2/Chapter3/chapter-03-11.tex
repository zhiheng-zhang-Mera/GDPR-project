...
\centering
\small
\caption{Revision 11 requirements and acceptance evidence}
...
This priority model avoids two extremes. It does not delay all prototype learning until every production activity is complete, and it does not allow a successful demonstration to erase unresolved authority or legal questions. Each gate authorises a named next activity. The Revision 11 candidate is designed to authorise controlled store-submission preparation, internal-track testing, and participant evaluation.
...
Aggregate windows create a double-counting problem. Suppose two records report counts for intervals that overlap. Without event identifiers, the system cannot know whether events in the shared interval appear in both counts. Summing them could manufacture a cross-window threshold. Revision 11 therefore aggregates only completed, non-overlapping windows for the same package and permission.
