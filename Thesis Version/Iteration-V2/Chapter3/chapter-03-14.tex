...
Table~\ref{tab:r14-requirements} defines the requirements used for implementation and evaluation. The table is intentionally concise; later subsections explain why each requirement is necessary and which failure it prevents.

\begin{table}[H]
\centering
\small
\caption{Revision 14 requirements and acceptance evidence}
\label{tab:r14-requirements}
...
F6 & Reject packs whose governance lifecycle, source, reviewer scope, or release scope is inconsistent. & Registry-load and mutation tests \\
...
P1 & Keep findings local, disable Android backup and remote updates, and remove application network permissions. & Privacy tests and merged manifest \\
...
This priority model avoids two extremes. It does not delay all prototype learning until every production activity is complete, and it does not allow a successful demonstration to erase unresolved authority or legal questions. Each gate authorises a named next activity. The Revision 14 candidate is designed to authorise controlled store-submission preparation, internal-track testing, and participant evaluation.
...
Aggregate windows create a double-counting problem. Suppose two records report counts for intervals that overlap. Without event identifiers, the system cannot know whether events in the shared interval appear in both counts. Summing them could manufacture a cross-window threshold. Revision 14 therefore aggregates only completed, non-overlapping windows for the same package and permission.
