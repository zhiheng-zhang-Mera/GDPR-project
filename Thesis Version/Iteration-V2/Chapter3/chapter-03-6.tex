...
\paragraph{Revision-3 scope.} The evaluated path comprises runtime-validated structured observations, the TypeScript compliance engine, AsyncStorage finding persistence, React Native presentation, and a WorkManager synthetic-event harness. Room, unrestricted third-party AppOps history, and physical 24-hour endurance are target-architecture components rather than evidence from the reported rerun.
...
\subsection{Implementation Scope Statement}

The architecture described in this chapter represents the complete design target for the framework. However, it is important to distinguish between the target architecture and the implemented prototype. The current implementation——described in detail in Chapter 4——realises a subset of this architecture: specifically, the TypeScript-based decision engine, the automated violation simulator, the local finding repository (using AsyncStorage), and the React Native presentation layer. The native acquisition components (\texttt{AppOpsManager} callback registration, WorkManager-based scheduling) and the relational persistence layer (Room database) are specified as target architecture for future development. This separation is deliberate: it allows the core decision logic and validation methodology to be developed and tested independently of the underlying data acquisition mechanism, which may vary across Android versions and deployment contexts.

The validation methodology presented in this thesis therefore focuses on the decision layer, using structured \texttt{PermissionAudit} observations supplied by the simulator or a future authorised acquisition module. Claims about cross-application permission history collection, WorkManager reliability over extended periods, and battery consumption are not supported by the current prototype and are identified as future work in Chapters 6 and 7.
...
\subsection{Target Data Acquisition: Historical Permission Audit Retrieval}

This section describes the \emph{target architecture} for data acquisition——the native-layer components that would provide real permission observations in a production deployment. These components are specified as design targets; the current prototype uses simulated observations supplied by the violation simulator.

The foundation of monitoring capability is the Android \texttt{AppOpsManager} system service. In Android 11 and later, the platform provides a public API to register a callback that is notified whenever an app accesses a protected data resource. The primary callback used is \texttt{onOpStarted} as it provides the earliest notification that a permission is about to be used. Each callback supplies the operation code (e.g., \texttt{OP\_FINE\_LOCATION}, \texttt{OP\_CAMERA}), the package name, and the user ID. The system clock time with millisecond precision is also recorded.
...
\subsection{Target Data Persistence and Deduplication}

The data persistence layer described here is the target architecture, using Room——Android's recommended abstraction layer over SQLite. The current prototype uses AsyncStorage for storing the latest findings only; the full relational schema remains a future extension.

The Room database schema would consist of three main tables:
\begin{itemize}
...
Deduplication would be handled at two levels: a composite unique constraint on \texttt{op\_events} for \texttt{(package\_name, permission\_code, timestamp, uid)} prevents duplicate insertion; and during aggregation, events older than 7 days are ignored to keep the database size bounded. A monthly cleanup job deletes events older than 30 days, maintaining a rolling window.

\subsection{Decision Model and Threshold Computation}
...
where $B_p$ is the configured baseline and $M_p=1.5$. The implemented baselines are 24 for LOCATION, 8 for MICROPHONE, and 4 for CONTACTS, yielding thresholds of 36, 12, and 6 respectively. These baselines are prototype values configured for demonstration; empirical calibration is deferred to future work. The excess and ratio are:
...
\subsection{Target Scheduling and Resource Management}

This section describes the target scheduling architecture using WorkManager. In the current prototype, scheduling is simulated within the TypeScript test harness; the native WorkManager layer remains a future extension.

The auditing pipeline would be orchestrated by WorkManager, which provides a robust API for deferrable background work. A \texttt{PeriodicWorkRequest} is defined with a 24-hour interval and a flex period of 4 hours, allowing the system to align execution with the device's optimal wake-up times. The worker uses \texttt{Result.success()} and \texttt{Result.retry()} appropriately; if the database is locked, retry with exponential backoff.
...
The comprehensive design ensures that the framework is both accurate and unobtrusive, providing a practical solution for end-user privacy compliance supervision while clearly delineating what is implemented, what is tested, and what remains as future work. The following chapter presents the concrete implementation of the TypeScript-based core, while the native-layer components described in this chapter remain specified as target architecture for future development.
\subsection{Revision-4 Physical-Device Protocol}

The physical protocol used an OPPO PERM00 handset with Android 12 (API 31), ARM64, a 1080 by 2400 physical display and ColorOS background controls enabled. The tested artefact was a release APK with an embedded JavaScript bundle, package \texttt{com.anonymous.mymobileapp}, version 1.0.0, target SDK 36 and SHA-256 \texttt{2F0B165738CBD3F84B99ADCB9C2405044D8B9524F5A1231DCDD0104E85D6E804}. Measurements comprised three force-stop cold starts, pre/post PSS samples, \texttt{dumpsys gfxinfo}, UI hierarchy capture, application log review and JobScheduler inspection. The handset remained USB-powered and charging, so battery percentage was deliberately excluded as an energy result.
