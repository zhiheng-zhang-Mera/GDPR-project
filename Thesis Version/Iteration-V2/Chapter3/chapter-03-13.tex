...
Table~\ref{tab:r13-requirements} defines the requirements used for implementation and evaluation. The table is intentionally concise; later subsections explain why each requirement is necessary and which failure it prevents.

\begin{table}[H]
\centering
\small
\caption{Revision 13 requirements and acceptance evidence}
\label{tab:r13-requirements}
...
This priority model avoids two extremes. It does not delay all prototype learning until every production activity is complete, and it does not allow a successful demonstration to erase unresolved authority or legal questions. Each gate authorises a named next activity. The Revision 13 candidate is designed to authorise controlled store-submission preparation, internal-track testing, and participant evaluation.
...
Aggregate windows create a double-counting problem. Suppose two records report counts for intervals that overlap. Without event identifiers, the system cannot know whether events in the shared interval appear in both counts. Summing them could manufacture a cross-window threshold. Revision 13 therefore aggregates only completed, non-overlapping windows for the same package and permission.
