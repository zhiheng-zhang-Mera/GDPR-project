...
\paragraph{Revision-8 visual-evidence limitation.} Accessibility labels and targeted lint reduce implementation risk but do not substitute for rendering. The next acceptance gate is an ARM64 build from a short Windows path with a complete locked dependency tree, followed by installation, UI-hierarchy comparison, screenshot review, interaction testing, and crash-buffer inspection on the connected device.

\paragraph{Revision-7 correction.} Source provenance is now restricted to \texttt{SIMULATOR}, \texttt{NATIVE\_BRIDGE}, or \texttt{IMPORTED}. Identical windows replace stored evidence, late observations cannot move the retention watermark backwards, and per-key history is bounded. These controls define deterministic replay semantics but do not authenticate the producer or deduplicate partially overlapping events.

\paragraph{Revision-6 correction.} The temporal detector now rejects duplicate aggregation across overlapping audit windows and accepts only completed non-overlapping history for cross-window evidence. This closes one false-positive mechanism but does not prove event-level deduplication: an acquisition layer must still assign stable event identifiers or non-overlapping collection cursors. The physical-device evidence remains a short 10-restart ARM64 run on one OPPO PERM00 device; it is not a multi-device endurance, battery, or 24-hour scheduling study.
...
\subsection{Revision-5 Limits of the Extended Campaign}

Ten process restarts improve evidence for ordinary lifecycle recovery but do not exercise reboot, Doze, app standby, low-memory killing, offline queue delivery, server reconciliation or a 24-hour periodic execution. The narrow 600~KB PSS range is not proof of leak freedom, and startup values from one handset cannot be generalised to other devices. AppOps output on the stock OPPO build exposed only application-owned modes and provided no independent third-party ground truth. Battery remained outside scope because the device was connected for ADB.

The new boundary suite demonstrates controlled rejection of the enumerated malformed cases; it is not exhaustive fuzzing of every JavaScript object shape or native bridge payload. Future validation should add property-based generation, AsyncStorage corruption/recovery tests, unplugged endurance, network fault injection for the offline queue and instrumented WorkManager execution across reboot and OEM power modes.
