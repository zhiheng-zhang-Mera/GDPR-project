...
Physical evaluation used one OPPO PERM00 device and a short acceptance window. The Revision 18 restored-state run comprised five cold starts between 1,203 and 1,819 ms without a crash-buffer entry. The same campaign exercised the handset's available 1.6 font scale and restored the original 0.9 setting. OEM process management, Android versions, accessibility settings, screen sizes, font scaling, languages, and long-run scheduling can change behaviour. These samples are an anomaly and reflow check, not evidence of a startup objective, 24-hour WorkManager reliability, battery impact, leak freedom, accessibility conformance, or multi-device stability.
...
The redesigned interface applies recognised accessibility and usability principles \cite{wcag22,androidaccess,iso924111}. Revision 16 adds text and shape alongside colour, minimum-size disclosure controls, a stable selected-navigation structure, and an explicit legal-source status register. Revision 17 reduces interpretive load by repeating the same three-stage reading sequence at overview and finding levels, and preserves agency through cancellable, consequence-first demonstration disclosure. Revision 18 adds source-tested reflow and a successful maximum-font physical matrix for selected states. Device captures support a positive implementation assessment. However, conformance-oriented inspection cannot establish comprehension, efficiency, satisfaction, trust calibration, anxiety reduction, or willingness to continue using the product. No screen-reader campaign, switch-access test, colour-vision simulation, multi-language large-font matrix, or participant study was completed. These remain required before making an accessibility-conformance or user-adoption claim.

\subsubsection{Interpreting the Engineering Result}

The evaluation is strongest when it is read as a chain of bounded claims rather than as a single pass or fail judgement. At the innermost layer, unit and integration tests establish that a specified input produces the specified state transition. At the next layer, release assembly and manifest inspection establish that the tested TypeScript is incorporated into an installable Android artefact with the recorded package properties. Physical-device interaction then establishes that selected routes render and selected controls respond on one hardware and operating-system combination. Revision 18's rendered matrix adds evidence for selected maximum-font reflow and retained reasoning structure, not for unseen devices, assistive technologies, or users. None of these layers automatically establishes the next one. A correct function may be absent from a package, an installable package may render poorly, and a coherent interface may still be misunderstood by users.
...
Revision 15 operationalises part of this distinction by recording each source as binding law, final guidance, or consultation material. This prevents the 2026 EDPB DPIA template and Article 6(1)(f) consultation text from silently acquiring the same status as the Regulation or final guidance \cite{edpblia2024draft,edpbdpia2026consultation}. The status is still maintainer-authored metadata. Without signed provenance, source retrieval, content hashing, and qualified review, the validator can enforce consistency but cannot establish authenticity or legal correctness.
