...
\subsection{How the Contributions Fit Together}
...
\subsection{Deployment and Evidence Roadmap}

The first deployment stage should be an owner-controlled internal track. Before upload, the owner should provision a protected upload key, build from a clean tagged revision, record checksums, verify the final merged manifest, and publish stable HTTPS privacy-policy and support pages. The Play Console Data safety answers must be reconciled against the exact binary and any introduced diagnostics. Test accounts are unnecessary for the current local-first design, but the listing should explain that simulated evidence is available for demonstration and that third-party visibility depends on authorised sources.

Internal-track evaluation should renew the functional and visual evidence using the store-delivered build rather than a locally installed sibling. Testers should cover first launch, the core review path, regulation-pack switching, evidence deletion, process death, update from the prior version, offline behaviour, large text, screen reader navigation, and representative screen sizes. Crashes and blocked tasks should be recorded with build identity. The aim is to find operational defects, not to claim population usability from a small group.

The second stage should add a closed participant study focused on comprehension and calibrated reliance. The study should include synthetic, authorised, missing-context, and malformed cases. Participants should explain what the result establishes, identify what is missing, and choose an appropriate next action. Success requires more than positive ratings: users should avoid both unwarranted alarm and unwarranted reassurance. Findings from this stage should drive terminology and hierarchy changes before a wider release.

The third stage should validate acquisition in one explicit authority model. A controlled event generator should establish ground truth across several Android and OEM combinations. Coverage, duplicates, timestamp error, scheduling delay, retention, restart recovery, battery, memory, and failure disclosure should be measured. The listing and in-app source descriptions must reflect the measured capability. If ordinary public-store deployment cannot provide useful external evidence, the product should position itself honestly as an imported-evidence reviewer or research demonstrator rather than imply device-wide monitoring.

The fourth stage should establish regulation-pack governance. Independent reviewers should approve the GDPR pack's citations, terminology, questions, and effective scope. A release process should require source records, fixture changes, localisation review, and deprecation notes. Only after one additional jurisdiction completes the same process should the product claim validated multi-region operation. Until then, the pack switch proves architectural flexibility.

The fifth stage should consider broader distribution. At that point, the maintainer needs vulnerability and dependency processes, incident handling, correction requests, user-support ownership, pack-update continuity, and periodic store-policy review. Metrics should be collected only when they are necessary and ethically justified. A privacy product should not undermine its value proposition through default behavioural analytics.

Each stage has a promotion criterion. Passing a stage authorises the associated claim and no broader one. This prevents pressure to translate technical progress into legal or market certainty. It also makes deferral acceptable: if production signing or independent legal review is unavailable, the candidate can remain a useful controlled artefact without pretending those gates have been met.

\subsection{Lessons for Privacy-Engineering Practice}
...
Future work should validate the GDPR pack independently, add a genuinely reviewed regional pack, improve authorised event-level acquisition, test multiple Android implementations over long durations, and conduct accessibility and participant studies. These activities should retain the Revision 10 claim-to-evidence discipline. Additional features are valuable only when their authority, provenance, and validation remain visible.
