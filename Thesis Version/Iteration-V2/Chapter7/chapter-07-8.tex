...
\paragraph{Revision-8 conclusion.} The interface source has a clearer evidence hierarchy, but the build blockade prevents device-level acceptance of the new pixels. This distinction is intentionally retained: source review and lint are engineering evidence, whereas visual quality requires a rendered, installed build.

\paragraph{Revision-7 conclusion.} The runtime trust boundary now validates provenance labels and defines bounded replay retention. Together with the claim-to-evidence matrix, this makes the implemented guarantees auditable without expanding them into acquisition authenticity, field accuracy, legal validity, or endurance claims.
