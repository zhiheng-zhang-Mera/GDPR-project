...
The thesis makes five supported contributions:

\begin{itemize}
    \item A non-intrusive, low-power permission call frequency audit using \texttt{AppOpsManager.getHistoricalOps()}, WorkManager, and Room, achieving <150 MB memory and <2\% extra battery drain. The framework operates entirely within the standard Android sandbox, respecting platform background limits while retrieving historical permission usage at a cadence balancing timeliness and energy efficiency. This was validated through accelerated emulator testing, demonstrating that the design targets are technically feasible.
    \item An intelligent decision engine with an ``expert baseline + dynamic threshold'' that adapts to application categories and historical variance, outperforming fixed-threshold approaches. The baseline is derived from statistical analysis of popular apps, and the dynamic threshold $\mu + k\sigma$ is tuned to balance precision and recall. The evaluation confirmed that the rule implementation is consistent and free of off-by-one errors.
    \item A modular, auditable compliance engine that explicitly supports GDPR Articles 5, 7, and 9, producing verifiable audit trails, supporting regulatory framework switching, and enabling independent verification of consent revocation effectiveness. Each \texttt{ComplianceReport} includes a cryptographic hash of the baseline configuration, ensuring audit trail integrity. The GDPR-oriented compliance context processing was validated through targeted test cases.
    \item An automated violation simulator that enables systematic, randomised stress testing of the detection logic, providing statistical confidence in precision and recall metrics. The Monte Carlo approach serves as an oracle, uncovering edge cases that manual testing would miss. The simulator was used to generate over 7,000 independent validation cases.
    \item A practical validation methodology combining Monte Carlo logical injection, automated simulation-based pressure testing, and a single real-device smoke test, balancing academic rigour with engineering feasibility. The two-iteration testing campaign——correctness followed by adversarial robustness——provides a template for rigorous evaluation of similar privacy tools.
\end{itemize}
