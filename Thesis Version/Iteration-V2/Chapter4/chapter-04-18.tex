...
The Settings screen separates preference, explanation, provenance, and deletion. Pack selection is restricted to registry entries. Selecting a pack displays its jurisdiction, version, description, caveat, and governance state. A source-register section renders every registered source with authority, checked date, and a distinct binding-law, final-guidance, or consultation label. The visual hierarchy therefore exposes the legal-source distinction added in Revision 15 instead of burying it in code. Clearing findings retains a destructive visual treatment and explanatory copy.
...
Accessibility labels are supplied for navigation and important actions. Icons and colour do not carry status alone: status text, a marker, and a structural rail remain available. The evidence disclosure is a separate 44-pixel-or-larger control rather than making an arbitrarily tall card one button. Bottom navigation exposes tab and selected-state semantics and adds a structural selected container. Source cards expose link labels that include source status and title. Revision 18 detects system font scale at or above 1.4 and changes dense Overview metrics, Findings summaries, and filters to a vertical reading order. It also removes forced package-name truncation, places finding status on its own row, and increases navigation height and spacing. These implementation facts and UI-tree evidence support an accessibility-oriented design claim, while screen-reader traversal, switch access, additional devices and languages, and assistive-technology user testing remain future acceptance work.

\subsection{Release Configuration and Privacy Controls}

The Revision 18 source sets minimum SDK 24, compile and target SDK 36, version code 7, and version name 1.6.0. The Expo configuration, JavaScript package metadata, settings footer, and Gradle declaration use the same version. Release tasks produced an ARM64 APK for controlled device installation and an AAB for Play delivery. The build reads upload-store path, passwords, and alias from environment variables. Because those values were absent, the accepted QA artefacts use the debug configuration and are not production-upload signed. Package inspection and physical installation were renewed for 1.6.0 rather than inherited from Revision 17.

The source manifest explicitly removes inherited Internet, network-state, and legacy external-storage permissions using manifest-merger directives. Inspection of the 1.6.0 APK found only foreground-service, wake-lock, boot-completed, and an application-scoped dynamic-receiver protection permission contributed by the background-work stack. Android backup and remote Expo updates remain disabled. A release-privacy contract checks version agreement, manifest removals, backup and update settings, and the absence of application network-client APIs. These package facts do not prove that every future dependency or connector will preserve the property.
