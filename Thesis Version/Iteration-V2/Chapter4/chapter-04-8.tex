...
\paragraph{Revision-8 interface refinement.} The Privacy accountability view now uses a consistent deep-blue evidence palette, an on-device review identifier, icon-supported metric cards, explicit scan and controlled-evaluation actions, and a structured empty state. These changes improve visual hierarchy and action recognition while preserving the warning language, legal caveat, filter semantics, and accessibility labels. The design does not imply that a more polished warning is more legally accurate.
...
The static baseline alone cannot capture individual user variation. A production study could estimate per-application baselines from separately governed historical data, but the evaluated prototype does not maintain a seven-day \texttt{daily\_aggregates} table or implement the adaptive formula below. The formula is retained as a target design requiring population definition, consent, retention controls, and independent calibration:
...
The implemented portion of this design is the validated TypeScript observation boundary, explicit warning rules, controlled simulator, local presentation path, and synthetic-event harness. Native acquisition authority, complete event aggregation, Room persistence, adaptive baselines, long-duration scheduling, and regulatory auditability remain target capabilities. The next chapter evaluates rule conformance and bounded integration evidence without treating these future components as completed.
\subsection{Revision-4 Implementation Reconciliation}

Release compilation exposed a reproducible manifest defect: \texttt{PrivacyContext.tsx} imports \texttt{@react-native-community/netinfo}, but the dependency was absent from \texttt{package.json}. Declaring version 11.4.1 corrected the build. The ARM64 release then compiled and installed successfully with the default React Native New Architecture.

The implemented native audit is narrower than several earlier design passages in this chapter. \texttt{PermissionAuditWorker} calls \texttt{unsafeCheckOpNoThrow} for this application's location, microphone and contacts operation modes and stores a capability snapshot in SharedPreferences. It does not call \texttt{getHistoricalOps}, collect third-party access counts, use Room, register an AppOps callback or run a foreground service. \texttt{AuditScheduler} registers one unique 24-hour \texttt{PeriodicWorkRequest} and an on-demand one-time request; no catch-up worker, 12-hour watchdog or measured drift correction is implemented. These executable facts supersede incompatible target-design wording above.

The previously stated Pixel 7, 85~MB, sub-1\% battery, microsecond callback and Room query figures have no matching raw artefacts in this repository and are withdrawn as results. The supported physical result is the OPPO campaign reported in Chapter~5. Because the phone was charging over USB, no battery-drain percentage is reported.
\subsection{Revision-5 Fail-Closed and Deterministic-Window Corrections}

Extended type-confusion tests showed that non-string purpose, controller identity or Article~9 fields could reach \texttt{trim()} and throw rather than returning a controlled rejection. The parser now validates those fields as strings and validates consent-withdrawn, special-category and user-initiated flags as booleans. Malformed contexts therefore return \texttt{INVALID\_CONTEXT} through the fail-closed API.

A second boundary test forced the simulator random source to zero. All generated timestamps then coincided, creating a zero-length audit window that the engine correctly rejected but the simulator had incorrectly produced. \texttt{simulationToAudit} now normalises the end of a degenerate window to at least one millisecond after its start. These changes were included in an ARM64 release APK of 32,776,788 bytes with SHA-256 \texttt{DA5C984CC4236ACD889597B98D98434375EBA3B5C15139DE4A436F78CE3C13BB}, installed successfully on the physical OPPO device.
