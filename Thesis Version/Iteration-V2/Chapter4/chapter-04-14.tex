...
The Android build sets minimum SDK 24, compile and target SDK 36, version code 3, and version name 1.2.0. The package manifest, Expo configuration, JavaScript package metadata, settings footer, and Gradle declaration use the same version. Release tasks produce an APK for controlled device installation and an AAB for Play delivery. The build reads upload-store path, passwords, and alias from environment variables. If they are absent, a local QA fallback signs with the debug configuration. The build can therefore be tested without placing secrets in source, while its output remains clearly distinguished from a production upload artefact.

The source manifest explicitly removes inherited Internet, network-state, and legacy external-storage permissions using manifest-merger directives. The final merged package requests only foreground-service, wake-lock, boot-completed, and an application-scoped dynamic-receiver protection permission contributed by the background-work stack. Android backup and remote Expo updates are disabled. A release-privacy contract also checks version agreement, manifest removals, backup and update settings, and the absence of application network-client APIs. These facts support the narrow claim that Revision 14 has no application network permission or evidence-upload path; they do not prove that every future dependency or connector will preserve that property.
