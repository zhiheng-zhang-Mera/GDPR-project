% ==================== Chapter 4: Detailed Design and Implementation (Integrated) ====================
\newpage
...
For a future native implementation, we evaluated several persistence alternatives. \textbf{Raw SQLite} offers maximum control but requires manual cursor handling, SQL string building, and thread management—this would increase development effort and the risk of SQL injection and concurrency bugs. \textbf{Realm} is an object-oriented mobile database that is faster for complex queries, but it adds over 5 MB to the APK size and does not support raw SQL, which we need for the date-based grouping and aggregation. Realm also lacks compile-time query verification, increasing runtime error potential. \textbf{Firebase Firestore} is a cloud-based NoSQL solution, but our framework operates entirely offline, and using Firestore would introduce unnecessary network dependency and privacy risks. \textbf{Room} strikes the optimal balance: it provides compile-time checked SQL, seamless coroutine integration, and low overhead. Its migration support is also robust, allowing schema evolution without data loss as the framework grows.
