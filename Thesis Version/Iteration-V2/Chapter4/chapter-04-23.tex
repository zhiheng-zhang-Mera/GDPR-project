...
\subsection{Revision 23 implementation: manifest-bound source bytes}

The implementation introduces a schema-version-5 content manifest and a byte-assessment module. Each artifact binds \texttt{sourceId}, \texttt{artifactId}, \texttt{url}, \texttt{mediaType}, \texttt{retrievedAt}, \texttt{byteLength}, and \texttt{sha256}. The canonicaliser validates these fields, sorts artifact values by code units, and serialises a fixed field order. SHA-256 is calculated over the unmodified byte array through the same audited cryptographic dependency used by the application. Attestation payload version 2 includes both the canonical source-record digest and the canonical source-content-manifest digest.

The runtime assessor exposes \texttt{VERIFIED}, \texttt{\seqsplit{MANIFEST\_NOT\_PROVIDED}}, \texttt{\seqsplit{ARTIFACTS\_NOT\_AVAILABLE}}, \texttt{\seqsplit{ARTIFACT\_MISSING}}, \texttt{\seqsplit{ARTIFACT\_LENGTH\_MISMATCH}}, \texttt{\seqsplit{ARTIFACT\_HASH\_MISMATCH}}, and \texttt{\seqsplit{NOT\_APPLICABLE}}. The compliance engine copies this assessment into every finding and adds offline verification of each official-source artifact to missing evidence when necessary. Historical findings without the new field migrate to \texttt{\seqsplit{NOT\_RECORDED}}; they are not silently reclassified as verified.

A reusable Node.js verifier accepts an artifact directory, imports the compiled manifest, reads every named file as bytes, and emits a machine-readable result. It exits unsuccessfully on any non-verified artifact. The six source files are evaluation inputs rather than application assets: committing or bundling them would enlarge the package and could confuse stored historical evidence with a live authority check. Settings therefore displays a three-link evidence chain---official document bytes, independent review signature, and production key governance---and states that a digest is neither a fresh download nor proof of legal correctness.

The associated key-governance document assigns reviewer, independent approver, release custodian, and incident-owner responsibilities. It requires controlled onboarding, two-person release, documented rotation and revocation, rollback handling, and published evidence receipts. The compiled application still uses an empty production trust store and an application-bundled static denylist. Thus the code expresses the gate and its operational preconditions without claiming that the repository supplies a production public-key infrastructure.
