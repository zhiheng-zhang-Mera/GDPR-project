...
The current EU GDPR pack is a \emph{technical candidate}: its official anchors, caveat, deterministic fixtures, and version are inspectable, but no independent qualified legal approval record exists. The Research Baseline is a non-legal demonstrator and cannot progress through the legal-review state. This lifecycle is a governance design in the thesis; the repository does not yet implement cryptographic pack signing, reviewer credentials, expiry enforcement, or remote revocation. Stating that gap prevents an architecture diagram from being mistaken for an operational approval system.
