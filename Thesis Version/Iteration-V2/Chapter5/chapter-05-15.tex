...
The evaluation separates six evidence layers: source conformance, deterministic rule behaviour, adversarial boundary behaviour, legal-evidence governance, Android package integration, and physical-device usability evidence. This separation prevents a passed unit test from being reported as device reliability or legal validity. The Revision 14 package acceptance record remains stored at \path{testing-report/ASSESSMENT-8-9-APP-V1.2.0.md}; Revision 15 adds current legal tests, renewed 1.3.0 release artefacts, manifest inspection, and a five-round physical-device record.
...
\midrule
\texttt{app-release.apk} & 59,397,535 & \texttt{\seqsplit{9F2D1051D15DCC936E8F5B703100FA0EAF52C13D293A225B0CAA6FAC31EC4A6A}} \\
\texttt{app-release.aab} & 29,602,702 & \texttt{\seqsplit{64944C4E7F3AF9E6B25DDEFADF17376F7ED0A4B3141E15DD78D98D34A09D2294}} \\
\bottomrule
\end{tabular}
\end{table}

The 1.3.0 merged manifest contained wake lock, boot rescheduling, foreground service, and an app-scoped dynamic-receiver protection permission. Internet, network-state, and external-storage permissions were absent. Inspection with \texttt{apkanalyzer} confirmed application ID \texttt{com.zhihengzhang.privacylens}, version code 4, version name 1.3.0, minimum SDK 24, and target SDK 36. APK Signature Scheme v2 verification passed; the signer remained the Android debug QA certificate rather than an owner-controlled production key.

\subsubsection{Physical-Device Evaluation}

The 1.3.0 networkless release APK was installed by ADB on one OPPO PERM00 device. Five force-stop and cold-start rounds completed. Median and mean \texttt{TotalTime} were 1,813 ms and 1,886.8 ms; sampled total PSS ranged from 96,341 to 96,819 KB; and the crash buffer contained no package-specific entry. The captured overview showed no obvious clipping, overlap, or broken navigation at the tested state. These are short-run observations, not a startup service-level objective, leak proof, battery evidence, legal validation, or broad performance claim.
...
The earlier simulated multidisciplinary screen assigned 95.1/100 under a release-candidate engineering rubric. Revision 15 does not carry that number forward as the optimisation baseline because it underweights the independent legal, participant, and publication evidence still absent. Following the project instruction, a stricter publication-target rubric resets the predecessor App--thesis system to 70/100. Under the same rubric, Revision 15 is assessed at 79/100: legal validity and governance 24/30, software evidence 22/25, interface and accessibility 15/20, user-psychology evidence 10/15, and academic communication 8/10. The nine-point movement reflects implemented source-status and legal-evidence controls plus renewed package and device evidence; it does not award points for external legal approval, participant outcomes, broad accessibility, long-duration reliability, or journal acceptance.
...
Interaction evidence included screenshots and UI hierarchy dumps for overview and settings, explicit radio selection state, and navigation state. The evaluator changed from EU GDPR to the visibly non-legal Research Baseline, force-stopped the application, cold-started it, and observed the Research Baseline still selected. EU GDPR was then restored and the overview captured again. The final ten-round run retained an empty package crash buffer. These checks validate selection persistence and rendered disclosure on one device; they do not validate the legal content of either pack.
...
A release tag should be immutable, while packs and policies use explicit version identifiers. If a later investigation finds an error, the project should publish a successor and a correction note rather than rewriting the historical evidence. This thesis follows the same principle by creating Revision 15 chapter files instead of replacing Revision 14. Preservation makes it possible to see which claims changed and why.
