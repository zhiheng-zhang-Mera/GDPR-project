...
\subsection{Revision-8 UI Baseline and Verification Boundary}

On 7 August 2026 an OPPO PERM00 running Android 12 was connected through ADB. The previously installed release opened successfully with a cold-start \texttt{TotalTime} of 1,048 ms, and the Privacy page was reached using coordinates derived from its UI hierarchy. A pre-change screenshot and hierarchy were retained. The revised TypeScript files passed targeted ESLint, and both compliance suites passed after source compilation.

The revised APK was not installed. Release assembly was blocked by a missing local \texttt{@react-native-community/netinfo} package and Windows native-build paths exceeding 260 characters. These are build-environment failures, not UI acceptance evidence. Consequently, Revision 8 records the source-level design and the old-release baseline but makes no claim that the new layout rendered correctly on the device.

\subsection{Revision-7 Claim-to-Evidence Matrix}

Table~\ref{tab:claim-evidence-r7} separates verified behaviour from inference boundaries. This matrix is normative for interpreting empirical language elsewhere in the thesis.

\begin{table}[H]
\centering
\small
\caption{Claim-to-evidence matrix for Revision 7}
\label{tab:claim-evidence-r7}
\begin{tabular}{p{0.23\textwidth}p{0.28\textwidth}p{0.37\textwidth}}
\toprule
Claim & Evidence & Boundary \\
\midrule
Rule conformance & 1,000 seeded logical cases plus boundary suites & Shared specification and controlled oracle; not field accuracy \\
Runtime rejection & Malformed type, unsafe integer, window, timestamp, context, and source tests & Covers enumerated inputs, not every bridge or parser \\
Temporal aggregation & Overlap, adjacency, replay, package, permission, and simulator-isolation tests & Aggregate windows lack event-level identifiers \\
Android execution & Ten short cold-start rounds on one OPPO PERM00 ARM64 device & Not 24-hour endurance, battery, OEM, or population evidence \\
Legal interpretation & Explicit missing-evidence fields and warning caveat & No automated finding determines GDPR infringement \\
\bottomrule
\end{tabular}
\end{table}

Revision 7 additionally validates the runtime source enumeration, replaces identical replay windows instead of accumulating them, uses the latest observed end time as a 24-hour retention watermark, and caps each package-permission history at 4,096 entries. The cap is a denial-of-service safeguard, not an empirically calibrated acquisition rate.

\subsection{Revision-6 Regression and Evidence Boundary}

On 7 August 2026 the compliance sources were rebuilt before execution. The original 1,000-round seeded suite and the extended boundary suite passed. Revision 6 adds explicit regression cases for overlapping and adjacent audit windows. Overlapping windows are excluded from rolling aggregation to prevent duplicate observations from producing a false \texttt{CROSS\_WINDOW} signal; adjacent completed windows remain eligible. Counts, audit times, timestamps, and retention periods must also be JavaScript safe integers. These results validate specified program behaviour only. They do not establish legal compliance, unrestricted third-party AppOps visibility, 24-hour WorkManager reliability, or population performance.

\subsection{Revision-3 Reproducibility Run}

On 3 August 2026 the TypeScript compliance suite was compiled with the repository's TypeScript 5.9 dependency and executed with the bundled Node runtime. A fixed linear-congruential seed generated 1,000 logical cases. Every fifth case was a below-threshold control and the remainder were simulator-labelled violations. The observed confusion matrix was (TP=800), (TN=200), (FP=0), and (FN=0), yielding precision (=1.0000) and recall (=1.0000). Security-boundary, temporal-detection, GDPR-accountability, communication, and dashboard-presentation assertions also passed.

The expected label is derived from the same published threshold specification as the engine. This is therefore a deterministic consistency and regression test, not an independent estimate of real-world malicious-app detection or GDPR infringement accuracy. No new physical-device endurance, battery, or memory measurements were collected.
...
\begin{table}[H]
\centering
\caption{First-iteration classification results by experimental round}
...
Following the adversarial findings, a fail-closed runtime input boundary was implemented, along with multi-scale temporal signals (burst rate and cross-window accumulation) and a GDPR-oriented compliance context. The enhanced engine was retested with a targeted suite. These repairs address the most critical vulnerabilities identified in the adversarial campaign.
...
The original daily count is retained as \texttt{DAILY\_TOTAL}. A sliding calculation now measures peak calls in every 60-second interval and emits \texttt{BURST\_RATE} when a permission-specific limit is exceeded. The burst threshold is set at 10 calls per minute for LOCATION, 5 for MICROPHONE, and 3 for CONTACTS——heuristic values requiring empirical calibration. A package-permission keyed \texttt{Map} also accumulates authorised imported or native windows over the preceding 24 hours. Multiple sub-threshold windows can therefore emit \texttt{CROSS\_WINDOW} when their combined count exceeds the daily threshold.
...
\subsection{Revision-4 ARM64 Physical-Device Results}

Table~\ref{tab:oppo-physical} reports the measurements collected from the release build on the OPPO PERM00 handset.

\begin{table}[H]
\centering
\caption{Physical-device release measurements on OPPO PERM00 (Android 12/API 31).}
\label{tab:oppo-physical}
\begin{tabular}{@{}ll@{}}
\toprule
Measure & Observation \\
\midrule
Cold-start \texttt{TotalTime} & 1,194; 993; 995~ms (median 995~ms) \\
Controlled 50-round corpus & TP 40; TN 10; FP 0; FN 0 \\
Controlled precision / recall & 100.0\% / 100.0\% \\
Pre-run mean PSS & 124,835~KB (approximately 121.91~MiB) \\
Post-run mean PSS & 139,706~KB (approximately 136.43~MiB) \\
Peak sampled PSS & 141,669~KB (approximately 138.35~MiB) \\
Rendered / janky frames & 39 / 3 (7.69\%, Android definition) \\
Frame percentiles & p50 11; p90 22; p95 34; p99 65~ms \\
Crash, ANR or React load failure & none observed in the captured interval \\
Periodic audit & 24-hour WorkManager target visible in JobScheduler \\
\bottomrule
\end{tabular}
\end{table}

The release PSS remained below the 150~MB design target during this short workflow. Mean PSS increased by 14,871~KB (11.9\%), which is not sufficient to diagnose or exclude a long-duration memory leak. The 50-round precision and recall values are exact only for the controlled oracle: they do not measure legal compliance or detection of unknown malicious applications. JobScheduler inspection proves registration of the periodic target, not completion after 24 hours, Doze timing, reboot recovery or OEM drift. The phone was USB-powered at 22\% battery and charging; consequently, the campaign provides a baseline temperature and state but no valid battery-consumption result.
\subsection{Revision-5 Extended Boundary and Restart Evaluation}

The logical suite was extended beyond aggregate threshold correctness to six previously uncovered dimensions: processing-context type confusion, timestamp/count/window consistency, exact burst boundaries for all three permission classes, package and permission history isolation, deterministic simulator extremes, and finite metric/presentation outputs. The original 1,000-round suite and all extended cases passed after the two corrections described in Chapter~4.

A reusable PowerShell/ADB harness then performed ten consecutive force-stop/cold-start cycles on the OPPO PERM00 release installation. Every cycle produced a live application process. \texttt{TotalTime} values were 1048, 998, 996, 1033, 987, 1033, 1057, 1009, 973 and 994~ms; the median was 1009~ms and the mean was 1012.8~ms. Per-cycle TOTAL PSS ranged from 127,705 to 128,305~KB, a 600~KB span without monotonic growth. No package-specific entry appeared in the crash buffer. This is scoped evidence of short restart reliability and bounded immediate memory variation, not a long-duration leak test.

After rebuilding with the corrections, the replacement release installed successfully and cold-started in 1736~ms. Its sampled PSS was 129,876~KB and log screening found no package-specific fatal exception, ANR or React Native error. The single post-install cold start is reported separately because installation and cache state make it non-comparable with the earlier ten-cycle distribution.
