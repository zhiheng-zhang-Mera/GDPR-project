...
\subsection{Revision 22 signed-bundle results}

The governance suite again passed 1,800 independently calculated pack cases and added paired probes for source-record mutation, signature mutation, an unknown signer, a revoked signer, attestation expiry, and source-order invariance. Removing the injected test trust anchor changed an otherwise valid signed fixture to \texttt{SIGNER\_NOT\_TRUSTED}; the real EU pack remained \texttt{NOT\_PROVIDED}. The fixed-seed 1,000-case corpus again produced 800 true positives, 200 true negatives, no false positives, and no false negatives. TypeScript, ESLint, extended compliance, accessibility-source, and release-privacy checks passed.

The final 1.10.0 release produced a 62,901,943-byte APK with SHA-256 \texttt{\seqsplit{4DAA5D59E7BDD3A5B7287AE345B6775A9B13604286146E37179D35E3DFE44253}} and a 31,777,577-byte AAB with SHA-256 \texttt{\seqsplit{A5BEB2946C52EC90B5A239DEB6F0E6E3B2DBCD7781FD2C7D8C2129EC6467AB6D}}. Gradle assembled both release formats, executed vital lint, and Metro bundled 1,077 modules. The final-hash APK installed on the OPPO PERM00. Five cold starts returned status ok with total times 931, 2,001, 878, 1,864, and 1,805 ms (median 1,805 ms); the crash buffer was empty.

Final-build settings evidence displayed the source-record digest, Ed25519 signature, trusted-key, non-revocation, and non-certification boundaries. Same-version finding evidence displayed synthetic provenance, the unresolved cryptographically verified legal-review requirement, and the instruction not to change access or confront a developer based on the card alone. Under the unchanged rubric, Revision 22 receives 93/100: legal governance 28/30, software evidence 23/25, interface and accessibility 20/20, user-psychology evidence 13/15, and academic communication 9/10. The additional point recognises executable authenticity and signer-lifecycle checks, not real legal approval.

\subsection{Revision 23 evaluation: offline source-content verification}

All six retrieved official PDF artifacts matched the schema-version-5 manifest. Their recorded byte lengths were 982,296; 329,009; 1,176,674; 1,137,918; 721,935; and 272,232 bytes. The corresponding SHA-256 digests begin \texttt{bd84e63f}, \texttt{d45afa0c}, \texttt{e026fbc2}, \texttt{4de1ee2d}, \texttt{59692ea4}, and \texttt{647995b5}. The offline verifier returned \texttt{VERIFIED} for each entry. A separate mutation probe changed one byte without changing file length; verification returned \texttt{ARTIFACT\_HASH\_MISMATCH} and a non-zero process result. This separates content integrity from the easier length check.

The fixed-seed 1,000-case suite again produced 800 true positives, 200 true negatives, no false positives, and no false negatives relative to the specified oracle. The independent 1,800-case governance oracle passed together with one-to-one manifest coverage, ordering invariance, missing-artifact, length-mismatch, same-length hash-mismatch, signature mutation, unknown-key, revoked-key, expired-key, and self-approval probes. Repository-wide lint, accessibility contracts over five files and twelve touch targets, release-privacy contracts over 28 source files, and whitespace checks passed.

The release build processed 1,079 Metro modules and 329 Android tasks. The 1.11.0 APK contained 62,916,583 bytes with SHA-256 \texttt{\seqsplit{5386eea1c9e8b7930830c89e2a21186e8042684a767784d7d9e637724ebca9e6}}; the AAB contained 31,784,243 bytes with SHA-256 \texttt{\seqsplit{865cae741af7c807e4390d07894333dee1a454a388549d89e1e6d305da1f794d}}. Installation on one OPPO PERM00 reported version code 12, version name 1.11.0, minimum SDK 24, and target SDK 36. Five cold starts returned status ok with total times of 1,934, 1,376, 1,811, 1,857, and 851 milliseconds; the median was 1,811 milliseconds and the crash buffer remained empty.

Device screenshots and UI trees show the three-link evidence chain, six-entry source register, current technical finding, and expanded evidence gap. They demonstrate rendering and inspectability on the tested handset, not user comprehension or accessibility conformance. The strict multidisciplinary self-assessment advances from 93 to 94 out of 100: legal compliance 29/30, software evidence 23/25, interface 20/20, user psychology 13/15, and academic quality 9/10. This is an internal rubric, not a legal opinion, university grade, journal decision, or deployment certificate.
