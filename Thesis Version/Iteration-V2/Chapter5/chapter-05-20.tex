...
The evaluation separates eight evidence layers: source conformance, deterministic rule behaviour, adversarial boundary behaviour, legal-evidence governance, Android package integration, physical-device behaviour, rendered-interface communication, and human-factors design contracts. This separation prevents a passed unit test from being reported as device reliability, human comprehension, or legal validity. Revision 20 renews the 1.8.0 artefacts and adds source-review deadline mutations, fail-closed expiry checks, and rendered checks for review state and dates.
...
\midrule
\texttt{app-release.apk} & 62,777,563 & \texttt{\seqsplit{08594A648AB1E050A683380DAA20B34D1022B45F631DE875E04E1A685D4EB909}} \\
\texttt{app-release.aab} & 31,705,224 & \texttt{\seqsplit{1C2B52EE6349F35AA6E5645808C12E6E33A550B052BD7ABD6759F9383A00F77B}} \\
\bottomrule
\end{tabular}
\end{table}

The 1.8.0 merged manifest contained wake lock, boot rescheduling, foreground service, and an app-scoped dynamic-receiver protection permission. Internet, network-state, and external-storage permissions were absent. Inspection confirmed application ID \path{com.zhihengzhang.privacylens}, version code 9, version name 1.8.0, minimum SDK 24, and target SDK 36. The signer remained the Android debug QA certificate rather than an owner-controlled production key.

\subsubsection{Physical-Device Evaluation}

The 1.8.0 networkless release APK was installed by ADB on one OPPO PERM00 device. Device captures showed final and consultation sources with separate lifecycle and project review dates. An expanded finding showed ``source review current'', assessed 11 August 2026, with the next review due 11 September 2026. Seven UI-tree assertions covering lifecycle, state, and dates passed. Five force-stop and cold-start rounds all returned status ok; total times were 743, 1,686, 1,702, 1,702, and 1,712 ms (median 1,702 ms), and the package crash buffer contained no matching entry. These are short-run single-device and maintainer-source observations, not source authentication, legal validation, participant comprehension, or broad-performance evidence.
