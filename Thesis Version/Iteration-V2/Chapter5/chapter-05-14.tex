...
Revision 14 added a second deterministic campaign of 1,800 cases across both packs and all three supported permission families. Its expected daily signal and threshold are calculated in a small reference function rather than by calling the production helper. Values at and immediately above each threshold confirm the strict comparison, while a fractional product confirms ceiling behaviour. Governance mutations reject engineering-only approval, a non-HTTPS legal source, and an unknown pack identifier. This improves oracle diversity and pack-contract coverage without turning generated cases into population evidence.
...
\midrule
\texttt{app-release.apk} & 59,393,187 & \texttt{\seqsplit{5AE418FD6260130672BC43B0E61F2C78D3BE2262866C75F1E126114087F8732E}} \\
\texttt{app-release.aab} & 29,600,915 & \texttt{\seqsplit{10B33E1D5A26AA629BAF36C40EC5ADBCDE7C2F4C0633BA0AA1B16C34C167AB84}} \\
\bottomrule
\end{tabular}
\end{table}

The final merged release manifest contained wake lock, boot rescheduling, foreground service, and an app-scoped dynamic-receiver protection permission. Internet, network-state, and external-storage permissions were absent. Inspection with \texttt{apkanalyzer} confirmed application ID \texttt{com.zhihengzhang.privacylens}, version code 3, version name 1.2.0, minimum SDK 24, and target SDK 36. Android backup and remote Expo updates were disabled.

\subsubsection{Physical-Device Evaluation}

The final networkless release APK was installed by ADB on one OPPO PERM00 device. Ten force-stop and cold-start rounds completed. Median and mean \texttt{TotalTime} were 1,690 ms and 1,718.7 ms; sampled total PSS ranged from 95,844 to 96,719 KB; and the crash buffer contained no package-specific entry. These are short-run observations, not startup service-level objectives, leak proof, battery evidence, or broad performance claims.
...
The simulated multidisciplinary screen assigned an expert-proxy product score of 95.1/100: visual hierarchy 19.5/20, trust and claim boundaries 19.7/20, functional coherence 18.8/20, ease and accessibility foundations 18.6/20, and willingness-to-use proxy 18.5/20. This clears the project-defined ``beautiful, trustworthy, and willing to use'' gate for controlled store preparation. It remains a heuristic judgement by the project evaluator, not a statistically generalisable user study or measured adoption intention.
...
A release tag should be immutable, while packs and policies use explicit version identifiers. If a later investigation finds an error, the project should publish a successor and a correction note rather than rewriting the historical evidence. This thesis follows the same principle by creating Revision 14 chapter files instead of replacing Revision 13. Preservation makes it possible to see which claims changed and why.
