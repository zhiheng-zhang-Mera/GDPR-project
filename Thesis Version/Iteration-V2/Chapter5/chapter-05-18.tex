...
The evaluation separates eight evidence layers: source conformance, deterministic rule behaviour, adversarial boundary behaviour, legal-evidence governance, Android package integration, physical-device behaviour, rendered-interface communication, and human-factors design contracts. This separation prevents a passed unit test from being reported as device reliability, human comprehension, or legal validity. Revision 18 renews the 1.6.0 artefacts and adds rendered checks for large-text reflow, complete package identity, own-row status, expanded reasoning, and readable-structure disclosure.
...
\midrule
\texttt{app-release.apk} & 62,772,235 & \texttt{\seqsplit{501279E40AA82725A555BD34B92C87A30AC30543CFD5F081EE88807481BA8CB9}} \\
\texttt{app-release.aab} & 31,702,694 & \texttt{\seqsplit{AFA6AD4BAAC40AFCFA60B365AB8252C086D2073E30FEC779838A28823306D4C7}} \\
\bottomrule
\end{tabular}
\end{table}

The 1.6.0 merged manifest contained wake lock, boot rescheduling, foreground service, and an app-scoped dynamic-receiver protection permission. Internet, network-state, and external-storage permissions were absent. Inspection confirmed application ID \path{com.zhihengzhang.privacylens}, version code 7, version name 1.6.0, minimum SDK 24, and target SDK 36. The signer remained the Android debug QA certificate rather than an owner-controlled production key.

\subsubsection{Physical-Device Evaluation}

The 1.6.0 networkless release APK was installed by ADB on one OPPO PERM00 device. The system font scale was moved from the original 0.9 to the available 1.6 maximum through the visible OEM settings interface. Device captures covered the Overview metric reflow, Findings summary and filter reflow, complete package identity, own-row status, expanded reasoning, and the Settings readable-structure disclosure without observed clipping in the inspected states. The original 0.9 scale was restored. Five subsequent force-stop and cold-start rounds completed in 1,203--1,819 ms, and the crash buffer remained empty. These are short-run single-device observations, not an accessibility-conformance, startup-objective, participant-comprehension, leak, legal-validation, or broad-performance claim.
...
The stricter publication-target rubric resets the predecessor App--thesis system to 70/100, assessed Revision 15 at 79/100, Revision 16 at 84/100, and Revision 17 at 87/100. Under the same rubric, Revision 18 is assessed at 89/100: legal validity and governance 24/30, software evidence 23/25, interface and accessibility 20/20, user-psychology design evidence 13/15, and academic communication 9/10. The two-point movement reflects implemented maximum-font reflow plus a renewed source--package--device--render claim chain. It does not award accessibility-conformance or human-outcome credit because no assistive-technology or participant study exists.
...
Revision 18 adds a physical large-text matrix from the installed 1.6.0 binary. At system \texttt{font\_scale=1.6}, dense metric, ledger, and filter rows switch to a vertical reading order; package identity is not forcibly truncated; status occupies a separate row; and navigation receives additional height. UI hierarchies and screenshots were captured for each state, and the device was restored to its original scale. This establishes the inspected reflow behaviour on one handset, not TalkBack quality, switch-access operability, multi-language resilience, assistive-technology usability, or WCAG conformance.
...
A release tag should be immutable, while packs and policies use explicit version identifiers. If a later investigation finds an error, the project should publish a successor and a correction note rather than rewriting the historical evidence. This thesis follows the same principle by creating Revision 18 chapter files instead of replacing Revision 17. Preservation makes it possible to see which claims changed and why.
