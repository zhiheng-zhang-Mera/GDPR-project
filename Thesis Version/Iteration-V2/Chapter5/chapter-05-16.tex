...
The evaluation separates seven evidence layers: source conformance, deterministic rule behaviour, adversarial boundary behaviour, legal-evidence governance, Android package integration, physical-device behaviour, and rendered-interface communication. This separation prevents a passed unit test from being reported as device reliability, usability, or legal validity. Revision 15 supplied the strengthened legal contract; Revision 16 renews the 1.4.0 release artefacts and adds a four-screen device matrix, non-colour status structure, a visible source-status register, and targeted interaction-state checks.
...
\midrule
\texttt{app-release.apk} & 59,407,715 & \texttt{\seqsplit{FB87A9B32E6F1A5FBC83F9EA35F0051D381524DBD0EE82DE3E7BCCA0CF58EBF7}} \\
\texttt{app-release.aab} & 29,607,554 & \texttt{\seqsplit{7EE5B3A1DFE4C6AFA42DAEB41BC1424A44732EC4BD8637547D52751F13DB3061}} \\
\bottomrule
\end{tabular}
\end{table}

The 1.4.0 merged manifest contained wake lock, boot rescheduling, foreground service, and an app-scoped dynamic-receiver protection permission. Internet, network-state, and external-storage permissions were absent. Inspection confirmed application ID \path{com.zhihengzhang.privacylens}, version code 5, version name 1.4.0, minimum SDK 24, and target SDK 36. The signer remained the Android debug QA certificate rather than an owner-controlled production key.

\subsubsection{Physical-Device Evaluation}

The 1.4.0 networkless release APK was installed by ADB on one OPPO PERM00 device. Five force-stop and cold-start rounds completed. Median and mean \texttt{TotalTime} were 1,941 ms and 1,998.2 ms; sampled total PSS ranged from 106,713 to 108,019 KB; and the crash buffer contained no package-specific entry. A final four-screen matrix covered the Overview, populated Findings, pack-selection, and source-register states without obvious clipping or overlap. These are short-run observations, not a startup service-level objective, leak proof, battery evidence, legal validation, or broad performance claim.
...
The earlier simulated multidisciplinary screen assigned 95.1/100 under a release-candidate engineering rubric. The stricter publication-target rubric instead resets the predecessor App--thesis system to 70/100 and assessed Revision 15 at 79/100. Under the same rubric, Revision 16 is assessed at 84/100: legal validity and governance 24/30, software evidence 23/25, interface and accessibility 19/20, user-psychology evidence 10/15, and academic communication 8/10. The five-point movement from Revision 15 reflects the visible source register, non-colour status cues, renewed release and device evidence, and correction of two state-transition defects. It does not award points for external legal approval, participant outcomes, assistive-technology conformance, long-duration reliability, or journal acceptance.
...
A release tag should be immutable, while packs and policies use explicit version identifiers. If a later investigation finds an error, the project should publish a successor and a correction note rather than rewriting the historical evidence. This thesis follows the same principle by creating Revision 16 chapter files instead of replacing Revision 15. Preservation makes it possible to see which claims changed and why.
