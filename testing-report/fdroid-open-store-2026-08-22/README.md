...
`fdroid-catalog-500-verified-final-495.json` contains the final 495 APK entries whose content SHA-256 and signing-certificate SHA-256 were both verified. Five catalog candidates were rejected during final verification; the retained catalog and census are the authoritative final snapshot.
...
`COMPLETION-AUDIT.md` is the authoritative requirement-by-requirement status, including the remaining gates for a lawful commercial corpus and a statistically valid same-corpus baseline comparison.
