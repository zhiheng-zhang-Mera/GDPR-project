...
export const FindingCard = memo(function FindingCard({ finding }: { finding: ComplianceFinding }) {
  ...
  const findingTitle = finding.temporalEvidence?.length ? finding.communication.title : PERMISSION_LABEL[finding.permissionType];
    ...
    <View style={styles.card}>
      ...
      <View style={styles.topRow}>
        ...
        <View style={styles.heading}>
          <Text style={styles.title}>{findingTitle}</Text>
      ...
      <View style={styles.metaRow}>
        <View style={styles.metaChip}><Ionicons name={finding.evidence.evidenceKind === 'CONTROLLED_DEMO' || finding.evidence.source === 'SIMULATOR' ? 'flask-outline' : 'phone-portrait-outline'} size={14} color={T.colors.muted} /><Text style={styles.meta}>{finding.evidence.evidenceKind === 'CONTROLLED_DEMO' ? 'Controlled device demo' : SOURCE_LABEL[finding.evidence.source]}</Text></View>
      ...
      <TouchableOpacity
        ...
        accessibilityLabel={`${expanded ? 'Hide' : 'View'} evidence for ${presentation.label}, ${findingTitle}, ${finding.packageName}`}
...
const styles = StyleSheet.create({
  ...
  temporalEvidence: { marginTop: 8, padding: 10, borderRadius: 11, backgroundColor: T.colors.mint, borderWidth: 1, borderColor: T.colors.mintStrong },
  temporalTitle: { color: T.colors.primaryDark, fontSize: 12, lineHeight: 17, fontWeight: '900' },
  receipt: { color: T.colors.muted, fontSize: 10, lineHeight: 15, marginTop: 4 },
