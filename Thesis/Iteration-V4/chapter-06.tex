...
The simulator is intentionally useful for reproducible demonstration, but it can inflate perceived confidence if its labels are mistaken for field ground truth. the final delivery mitigates this by preserving synthetic provenance through the repository and interface. The remaining risk is user over-reliance on any precise metric; future builds should consider hiding precision and recall outside an explicitly labelled research panel.
...
\subsection{Iteration V4 residual limits}

Persistence improves continuity only within one installed application and the active maximum window. It is not tamper resistance, backup continuity, a chain of custody, or a substitute for an authorised observation source. Clock rollback, data clearing, uninstallation, damaged storage, pack changes, and operating-system limits remain fail-closed boundaries. The controlled route verifies transport and presentation in a test build, not sensing, collection, third-party AppOps history, or a legal violation. CI demonstrates repeatable engineering regression checks; it does not replace device diversity, independent legal assessment, or field validation.
