...
\subsection{Iteration V4 conclusion}

V4 strengthens the reproducibility chain while retaining its epistemic boundary: a profile-derived, Debug-only controlled device fixture has exercised the ordinary native evidence path on one OPPO PERM00 device, a minimal ledger provides same-pack restart continuity, and CI prevents quiet regression of those contracts. These results are engineering evidence. They do not convert a synthetic fixture into an observed event, a technical signal into legal advice, or one device run into general deployment validity.
