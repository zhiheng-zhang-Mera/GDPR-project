...
\subsection{Iteration V4 verification protocol}

V4 adds deterministic fixtures for both the GDPR and non-legal research packs, proving that a controlled fixture is compiled from the mounted profile rather than a fixed GDPR rule. The restart test compares the evidence receipt after a simulated process restart with an uninterrupted evaluation. Negative tests reject cross-pack snapshots, expired state, and forged deduplication keys. A release-privacy contract checks that the native controlled method is gated by \texttt{BuildConfig.DEBUG}; the JavaScript bridge delegates this decision to that native boundary so an offline debug bundle cannot suppress a valid debug-only test. Continuous integration runs these checks from a clean dependency installation. A physical-device acceptance remains limited to the actual build, installation, UI tree, screenshot, logcat, and crash-buffer evidence collected for that build; a controlled card is synthetic and cannot be reported as an observed privacy event.

\subsection{Loadable formal policy and commercial-APK baseline comparison}

The final iteration separates a second policy family from the temporal evaluator. A loadable constraint declares a conjunction of evidence predicates, the further contextual evidence that is required, legal references, a rationale, and the only permitted result: a review request. The predicate vocabulary distinguishes static APK capabilities such as \texttt{MANIFEST\_LOCATION} and a tracker code signature from time-scoped observed facts and from context that an APK cannot supply, including purpose, lawful basis, transparency, minimisation, retention, Article~9 condition, and DPIA screening. Invalid policy data fails closed before a pack is mounted. This makes the legal-to-logic mapping inspectable and reusable without turning a missing document or a declared permission into a legal conclusion.

On 22 August 2026, a read-only batch examined the installed base APKs of Meituan, Microsoft Edge, and WPS Office on the same OPPO PERM00 handset. Android SDK \texttt{apkanalyzer} supplied manifest and bounded DEX-package evidence. A second baseline applied a deliberately limited offline subset of the Exodus Privacy code-signature method, under which a signature indicates embedded code rather than effective tracking \cite{exodusprivacy2026}. Privacy Lens then evaluated only derived predicates against the loaded GDPR constraints. Table~\ref{tab:commercial-apk-baseline} records the bounded result.

\begin{table}[H]
\centering
\small
\caption{Read-only commercial APK baseline comparison; static observations are not GDPR findings}
\label{tab:commercial-apk-baseline}
\begin{tabular}{p{0.20\textwidth}p{0.14\textwidth}p{0.20\textwidth}p{0.34\textwidth}}
\toprule
Application & Manifest permissions & Baseline result & Formal-policy output \\
\midrule
Meituan & 146 & DEX tracker listing exceeded the bounded collector; no tracker absence inferred. & Location capability requested purpose, lawful-basis, transparency, minimisation, and retention evidence. \\
Microsoft Edge & 60 & Offline signature subset matched Adjust; embedded code only. & Location and tracker constraints requested review evidence; no violation asserted. \\
WPS Office & 11 & No match in the limited completed signature subset. & No constraint antecedent was satisfied; this is not a compliance result. \\
\bottomrule
\end{tabular}
\end{table}

The comparison is intentionally asymmetric. The Android SDK baseline provides static declaration and code-package evidence; the signature baseline provides a limited embedded-code indicator; Privacy Lens supplies a versioned legal trace and an explicit missing-evidence request. None observes runtime collection, server destinations, actual tracker use, controller identity, legal applicability, or consent. The results are retained with APK hashes, tool-method status, and no APK binaries, so another authorised reviewer can repeat the bounded experiment without treating it as a legal audit. This evidence strengthens reproducibility and comparative interpretation, not a claim that any commercial application infringed the GDPR.

The policy surface is extended from evidence predicates to a typed information-flow graph. A pack can now declare source categories, sink categories, legal references, contextual predicates, and the sole output \\texttt{REVIEW\\_REQUIRED}. Nodes retain a process identifier and edges preserve direct calls, callbacks, reflection, Binder, content-provider, and worker mechanisms. Evaluation performs bounded reachability over the typed graph, reports whether the witnessed path crosses a process boundary, and records whether an authorised runtime trace observed the corresponding permission. Obfuscated identifiers do not change the typed source, sink, or topology. This is a formal reachability-and-obligation model, not a claim that potential reachability proves collection, disclosure, or illegality.

The implementation has adversarial conformance fixtures for an obfuscated reflection path, an authorised dynamic-permission trace, and a Binder/worker cross-process path, plus a negative fixture and malformed-graph rejection. Against that deliberately constructed four-case oracle, the detector produced (TP=3), (FP=0), (TN=1), (FN=0), precision (=1.0000), recall (=1.0000), and (F1=1.0000). These are regression measurements of the stated formal semantics only; the fixture labels are not independent legal ground truth and must never be pooled with field accuracy.

For a recognised static-flow baseline, FlowDroid 2.15.1 was invoked on the retained WPS Office base APK using a versioned source-and-sink definition, Android platform directory, reflection enabled, bounded callbacks, timeouts, and two analysis threads \\cite{flowdroid2026}. The command completed in 89.5 seconds but produced no XML result artifact. The receipt therefore records \\texttt{COMPLETED\\_NO\\_RESULT\\_ARTIFACT}, not zero flows. This is an important reporting control: a tool invocation, a static potential flow, a tracker signature, and a formal review request are different evidence types.

\subsection{Pre-registered authorised 100-application protocol}

The repository contains an executable corpus contract for exactly 100 distinct Android commercial applications, each with a lawful store URL, package identifier, locally authorised APK or split set, and SHA-256 digest. Validation-only mode verifies all 100 entries and all digests without contacting ADB. Device execution requires both \\texttt{--execute} and \\texttt{--allow-device-installs}; it refuses any package already present, installs only verified input files, captures process count, PSS memory, battery-statistics hash and any available estimated power value, resolves a launch activity, force-stops, uninstalls, and verifies removal. Thus it cannot silently replace a user's application or record unavailable energy as zero.

The registered field comparison will run Privacy Lens, Android SDK manifest inspection, Exodus-style signature evidence, FlowDroid, and an available inter-component static-flow baseline on the same hashed corpus and pinned tool configurations. Independent per-application labels, produced under a disclosed annotation protocol, are required before calculating TP, FP, TN, FN, precision, recall, and F1. Wall time, successful analysis fraction, PSS, and available energy readings will be reported separately by tool and device; unobservable values remain missing. Until a lawful 100-application corpus and independent labels are supplied, this protocol is infrastructure and preregistration, not a completed population study or a demonstration of superiority.

As a separate device-compatibility experiment, 100 precompiled DroidBench APK entries were digest-verified and submitted sequentially to an OPPO PERM00 Android~12 device. The runner handled only the explicit OPPO ``continue installation'' page, declined the optional enhanced-protection setting, bounded an unresponsive installation at thirty seconds, and checked every catalogue package after completion. The result was 69 PackageManager rejections and 31 timeouts, with zero completed installations, zero removal failures, and zero remaining benchmark packages. Consequently there are no runtime PSS or energy observations for this device/benchmark combination. This is neither a negative privacy finding nor an algorithm false negative. It establishes a reproducible compatibility boundary: an old static micro-benchmark suite cannot by itself supply modern-device runtime overhead or commercial-application accuracy evidence.
