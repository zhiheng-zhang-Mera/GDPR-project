...
The implementation exports the bounded temporal ledger only to app-private storage and tags it with the mounted pack identity and version. Rehydration is therefore not a general history import. The Debug Android module accepts a controlled JSON fixture only when the build flag permits it, verifies its explicit label and generic observation vocabulary, and emits the existing passive-audit event. JavaScript independently refuses the call outside Debug Android. The existing context processes the emitted event through validation, ledger, mapping, evidence receipt, finding construction, and local presentation; screen code cannot manufacture a reassuring result.
