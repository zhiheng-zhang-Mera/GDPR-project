...
The physical-device runner processed 121 distinct package identities over two non-overlapping receipts. It completed 112 install--launch--force-stop--uninstall cycles and verified that every completed package was removed. There were nine failures: eight installer timeouts and one pre-existing-package protection. No removal failure was observed. The protection is important: a package present before the runner reaches it is neither replaced nor uninstalled, even on the controlled test device.

An expanded 298-APK static Manifest and DEX-reference census completed without a static failure. It found 32 declared multi-process applications, 49 sensitive-permission-plus-background-execution review signals, and 42 sensitive-permission-plus-network review signals. These results establish corpus diversity and static review coverage only. They do not establish a runtime source-to-sink flow, controller purpose, lawful basis, necessity, or GDPR infringement. The information-flow model was instead stress-tested through controlled adversarial graphs: obfuscated node identifiers, reflection edges, dynamic runtime-permission evidence, and Binder/Worker cross-process paths preserve a typed review prompt; malformed graph edges fail closed. Its $(TP,FP,TN,FN)=(3,0,1,0)$ result is a fixture-oracle conformance result, not a field estimate.

The follow-up device receipt collected post-launch memory for ten non-overlapping packages, with mean PSS 61,060 KB and mean end-to-end workflow time 27,386 ms. Across both receipts, the OEM runtime-permission UI was observed but not granted in 40 package runs, not observed in 72, and unavailable for the nine failed runs. This shows that the runner preserved the dynamic-permission prompt boundary; it does not establish that each application requested, received, or used a permission. Likewise, the real-app corpus has no independent legal ground truth, so precision, recall, F1, TP, FP, TN, and FN are not computed for it. This protects the distinction between reproducible engineering execution and a validated legal or privacy classifier.

\subsubsection{Public Static-Analysis Baseline}

To make the baseline evidence auditable rather than merely executable, seven FlowDroid 2.15.1 receipts were joined to public DroidBench descriptions at pinned revision \texttt{a57fa6f42f278591695672f1aa8b37c275139370}. Three receipts emitted a result artifact and were scored against their published source-to-sink outcome: $(TP,FP,TN,FN)=(2,1,0,0)$, precision $=0.666667$, recall $=1.000000$, and $F1=0.800000$. Four completed runs emitted no result artifact and were retained as unresolved rather than converted to zero-flow negatives. This is a small FlowDroid-only benchmark result, not a score for Privacy Lens, not a result for the open-source app-store corpus, and not a GDPR-infringement classifier evaluation. Privacy Lens currently lacks an APK-to-typed-flow extractor operating over these same cases, so no cross-tool superiority claim is warranted.
