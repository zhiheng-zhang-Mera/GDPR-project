...
The follow-up device receipt collected post-launch memory for ten non-overlapping packages, with mean PSS 61,060 KB and mean end-to-end workflow time 27,386 ms. Across both receipts, the OEM runtime-permission UI was observed but not granted in 40 package runs, not observed in 72, and unavailable for the nine failed runs. This shows that the runner preserved the dynamic-permission prompt boundary; it does not establish that each application requested, received, or used a permission. Energy in mAh was unavailable. The device was USB-powered and exposed zero battery current, so a battery-statistics value could not be de-biased into application energy. Missing energy was retained as unavailable rather than recorded as zero. Likewise, the real-app corpus has no independent legal ground truth, so precision, recall, F1, TP, FP, TN, and FN are not computed for it. This protects the distinction between reproducible engineering execution and a validated legal or privacy classifier.
