...
The release APK was installed by ADB on one OPPO PERM00 ARM64 device. The installed package reported \texttt{com.zhihengzhang.privacylens}, versionName \PrivacyLensVersion{}, versionCode 16, minSdk 24, and targetSdk 36. Launcher cold start succeeded. The Android crash buffer was empty after the recorded 24 August accessibility and decision-pause flow. This is an observation for one installed build and interval, not a long-run crash-rate estimate.
...
To test the engineering workflow beyond the project APK, a separate authorised corpus was constructed from the official F-Droid repository. This is an \emph{open-source app-store} corpus, not a closed-source commercial-store sample. The final hash-pinned static census contains \FDroidCensusN{} APKs; the device subset and its exclusion boundary are separately receipt-owned. The catalogue, hash boundary, and static census are recorded in \path{testing-report/fdroid-open-store-2026-08-22/}.

The physical-device runner processed \DevicePackagesN{} distinct package identities over three non-overlapping receipts. It completed \CompletedWorkflowsN{} install--launch--force-stop--uninstall cycles and retained \FailedDeviceWorkflowsN{} failures: 26 installer timeouts, two no-launchable-activity cases, one package-manager rejection, and one pre-existing-package protection. No removal failure was observed. The protection is important: a package present before the runner reaches it is neither replaced nor uninstalled, even on the controlled test device.
