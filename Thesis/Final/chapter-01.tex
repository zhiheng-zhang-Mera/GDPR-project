...
The method combines constructive and empirical elements. The constructive element is the artefact: the evidence contract, temporal ledger, rule-pack compiler, typed policy graph, repository, FlowDroid adapter, Android bridge, and interface. The empirical element consists of deterministic and curated mutation tests, adversarial boundary cases, a hash-pinned \FDroidCensusN{}-APK static census, \DevicePackagesN{} bounded package workflows on one device, static-analysis receipts, installed-package inspection, UI hierarchies, screenshots, and filtered runtime logs. The method does not include a controlled participant experiment or completed independent legal mapping review; those omissions are recorded as limitations rather than replaced with speculative conclusions.
...
The \textbf{architectural contribution} is an evidence-bounded pipeline in which admission, temporal preparation, typed graph construction, rule evaluation, persistence, projection, and presentation preserve the same claim boundary. The \textbf{semantic contribution} is a set of executable safety properties for malformed input, replay, overlap, provenance mixing, pack drift, restart, and missing outputs, with curated negative implementations showing that the suite detects the registered weakenings. The \textbf{methodological contribution} is a claim-to-evidence record that distinguishes oracle conformance, interoperability, external engineering execution, device behaviour, legal validity, and human comprehension. The \textbf{practical contribution} is a local-first Android prototype whose Overview, Findings, and Settings journeys expose source, selected pack, missing evidence, and data controls without requesting sensitive runtime permission.

\begin{table}[H]
\centering
\caption{Contribution, evidence, and claim boundary}
\label{tab:contribution-map}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}}
\toprule
Contribution & Gap addressed & Implementation & Evaluation evidence & Claim boundary \\
\midrule
C1 Evidence-bounded architecture & Source authority and legal context disappear between observation and UI & Admission, temporal preparation, evaluation, persistence, projection, and presentation retain one typed record & Traceability checks, package evidence, UI hierarchy, and claim-evidence matrix & Does not create unavailable acquisition authority or legal facts \\
C2 Regulation-owned temporal abstraction & Jurisdictional assumptions become hidden application logic & Versioned pack, mapping compiler, generic order-agnostic evaluator, and historical pack identity & Exact boundaries, 720 permutations, pack switching, and cross-pack rejection & Software replaceability is not comparative-law validity \\
C3 Fail-closed governance chain & Missing or malformed evidence can become false reassurance & Closed sources, safe numbers, bounded ledger, attestation, trust, rollback, and witness gates & Adversarial suites and nine of nine registered mutant detections & Curated mutants do not prove completeness or production security \\
C4 Typed claim separation & Technical signals are easily inflated into legal verdicts & Evidence, signal, review obligation, and prohibited conclusion occupy distinct output domains & Formal witnesses and adversarial information-flow fixtures & A review obligation is not proof of infringement or compliance \\
C5 Reproducible claim-boundary evaluation & Unlike evidence is often collapsed into one score & Hash-pinned manifests, missingness-preserving summaries, receipts, and one canonical reproduction path & F-Droid, FlowDroid, OPPO, deterministic, and source-quality records & Supports bounded engineering reconstruction, not population, human, or legal accuracy \\
\bottomrule
\end{tabular}
\end{table}
