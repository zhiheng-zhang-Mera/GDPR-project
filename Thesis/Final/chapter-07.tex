...
\textbf{RQ4: evidence strength and external validity.} Deterministic and adversarial suites support conformance to declared semantics. The hash-pinned F-Droid census supports processing of heterogeneous open-source APK artifacts. The complete 233-package device denominator, including 30 retained failures and 98 PSS observations, supports only bounded single-device engineering statements. FlowDroid receipts support interoperability and preserve missing XML as missing evidence. None of these results establishes Android-population prevalence, legal compliance, human comprehension, unrestricted Android visibility, long-run reliability, production signing, or store acceptance.
...
Future work should validate the GDPR pack independently, add a genuinely reviewed regional pack, improve authorised event-level acquisition, test multiple Android implementations over long durations, and conduct accessibility and participant studies. These activities should retain the the final delivery claim-to-evidence discipline. Additional features are valuable only when their authority, provenance, and validation remain visible.
...
The delivered system strengthens the reproducibility chain while retaining its epistemic boundary: a profile-derived, Debug-only controlled device fixture has exercised the ordinary native evidence path on one OPPO PERM00 device, a minimal ledger provides same-pack restart continuity, and CI prevents quiet regression of those contracts. The final evidence also includes two fixed-seed, 150,010-assertion stress executions across both registered packs. These results are engineering evidence. They do not convert a synthetic fixture into an observed event, a technical signal into legal advice, or one device run into general deployment validity.
