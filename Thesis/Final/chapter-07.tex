...
The fifth contribution, the verification record, connects artefacts to claims. Deterministic fixtures, nine registered safety properties and curated mutants, typed-policy fixtures, build outputs, corpus and missingness summaries, FlowDroid provenance, device screenshots, and unresolved review gates are not collapsed into one test total. This makes the evaluation auditable and limits accidental overstatement. It also defines a repeatable path for future releases: every material change should identify which evidence layers must be renewed.
...
Future work should complete independent validation of the GDPR pack, add a genuinely reviewed regional pack, improve authorised event-level acquisition, test multiple Android implementations over long durations, and conduct accessibility and participant studies. These activities should retain the thesis's claim-to-evidence discipline. Additional features are valuable only when their authority, provenance, and validation remain visible.

The programme should extend the existing formal and property-based techniques for evidence admission and replay. The registered suite covers idempotence, bounded storage, monotonic watermarks, source isolation, reordered independent windows, pack isolation, and missing outputs; its curated mutants should be complemented by automatically generated and independently designed mutants. A separately implemented oracle would reduce the correlated-defect risk of deriving implementation and expected values from one specification.
