...
The comparison matrix covers specialised but non-equivalent analytical objects. FlowDroid predicts potential static source-to-sink paths, while TaintDroid records selected flows under a modified runtime \cite{flowdroid2014,taintdroid2010}. Exodus matches embedded tracker signatures and expressly cautions that a match does not prove effective use \cite{exodus2026}. PoliCheck relates observed flows to entity-sensitive policy statements, and contextual-integrity analysis exposes omitted or ambiguous transmission parameters \cite{policheck2020,shvartz2019}. Permission-dialog studies measure attention, comprehension, expectation, and decisions that source inspection cannot establish \cite{felt2012,bonne2017}. Android's Privacy Dashboard supplies an operating-system-owned history for supported permissions and windows, not purpose or legal meaning \cite{androiddashboard2026}. These works motivate distinct evidence types; none is treated as a weaker implementation of one common classifier.
...
\subsection{Mapping governance}

The delivered system makes the five GDPR temporal profiles reviewable as rule cards. Each card records the risk assumption, window and combination, cited reference, counterexample, unknown facts, and a non-assurance review state. This documentation is governance evidence rather than independent legal review: a missing or stale attestation still prevents a reassuring legal conclusion.
