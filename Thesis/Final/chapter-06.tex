...
The deterministic confusion matrix measures agreement with a defined threshold oracle. It does not measure whether that threshold identifies unlawful processing. Manifest declarations, DEX references, static paths, runtime traces, and policy prompts are also different analytical objects: none is silently promoted to proof of another. This separation narrows construct claims to what each evidence type represents.
...
\subsubsection{Human Validity}

The redesigned interface applies recognised accessibility and usability principles \cite{wcag22,androidaccess,iso924111}, and the rendered evidence supports a bounded engineering assessment of labels, touch targets, evidence caveats, and the pause between signal and action. However, conformance-oriented inspection cannot establish comprehension, efficiency, satisfaction, trust calibration, or willingness to continue using the product. No completed screen-reader campaign, switch-access test, colour-vision simulation, large-font matrix, or participant study is available. These are required before making an accessibility-conformance or user-adoption claim.

\subsubsection{Reproducibility Boundary}
...
\subsection{Temporal-alert limits and next validation}
...
Future regulation packs should be authored through controlled legal and engineering review, with versioned source material, traceable interpretation notes, test fixtures for matches and non-matches, and a staged rollback path. A rule author must set the window and required combination for the assessment item and document why those parameters are appropriate; the generic engine should not invent a default. Pack loading must continue to validate the converter boundary so that unsupported observation types, empty legal references, unsafe counts, or implausible windows cannot activate accidental alerts.

External validation remains required before any broader claim. It includes coverage studies against documented Android behaviours, independent legal analysis of each mapping, usability work on notice comprehension and alert fatigue, accessibility review on target devices, security review of receipt handling, and field observation under an approved data-minimisation protocol. The system is a reproducible, decoupled notification prototype: it makes a rule-owned combination visible for human review while leaving legal judgment and consequential intervention outside the App.

\subsection{Residual limits}
