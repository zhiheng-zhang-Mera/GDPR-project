...
const SOURCE_STATUS_LABEL = {
  ...
  CONSULTATION_MATERIAL: 'Consultation material',
} as const;
  ...
  const legalReviewBody = legalReview.state === 'CURRENT'
    ...
    : legalReview.state === 'EXPIRED'
      ...
      : legalReview.state === 'NOT_PROVIDED'
        ? `No independent qualified legal-review attestation is recorded for ${selectedPack.versionLabel}. Reassurance requires verified source bytes, matching record and content-manifest digests, an Ed25519 signature, and a valid non-revoked trusted key.`
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.factCard}>
          ...
          <View style={styles.divider} />
          <Fact icon="pause-circle-outline" title="Private decision pause" body="Decision-pause answers exist only in the open card, are cleared when the evidence card closes, and are never stored or uploaded." />
          <View style={styles.divider} />
        ...
        <Text style={styles.sectionTitle}>Legal review gate</Text>
        <View accessibilityRole="summary" accessibilityLabel={`${legalReviewLabel(legalReview.state)}. ${legalReviewBody}`} style={[styles.attestationCard, legalReviewWarning && styles.attestationCardWarning]}>
          <View style={[styles.attestationIcon, legalReviewWarning && styles.attestationIconWarning]}><Ionicons name={legalReview.state === 'CURRENT' ? 'shield-checkmark' : 'shield-outline'} size={22} color={legalReviewWarning ? T.colors.danger : T.colors.primary} /></View>
          <View style={styles.attestationCopy}><Text style={[styles.attestationTitle, legalReviewWarning && styles.attestationTitleWarning]}>{legalReviewLabel(legalReview.state)}</Text><Text style={styles.attestationBody}>{legalReviewBody}</Text><Text style={styles.attestationBoundary}>Project governance control · not a GDPR certification or statutory four-eyes requirement.</Text></View>
        </View>
        ...
        {selectedPack.sources.length > 0 ? (
          <View style={styles.sourceList}>
            {selectedPack.sources.map((source) => {
              ...
              const artifact = selectedPack.governance.sourceContentManifest?.entries.find(({ sourceUrl }) => sourceUrl === source.url);
              return (
                <TouchableOpacity key={source.url} accessibilityRole="link" accessibilityLabel={`Open ${SOURCE_STATUS_LABEL[source.status]} source: ${source.title}. ${sourceLifecycleLabel(source.lifecycle)}. ${source.versionLabel}. ${sourceReviewLabel(sourceReviewState(source))}; review due ${source.reviewDueAt}. ${artifact ? `Offline manifest digest recorded from ${artifact.retrievedAt}; source bytes are not bundled in this app.` : 'No offline content digest recorded.'}`} accessibilityHint="Opens the official source in your browser" onPress={() => void Linking.openURL(source.url)} style={styles.sourceCard}>
                  ...
                  <Text style={styles.sourceVersion}>{source.versionLabel}</Text>
                  <Text style={styles.sourceLifecycle}>{sourceLifecycleLabel(source.lifecycle)}</Text>
                  <Text style={styles.sourceMeta}>{source.authority} · status checked {source.checkedAt}</Text>
                  <Text style={[styles.sourceReview, sourceReviewState(source) === 'REVIEW_DUE' && styles.sourceReviewDue]}>{sourceReviewLabel(sourceReviewState(source))} · review due {source.reviewDueAt}</Text>
                  {artifact ? <Text style={styles.sourceDigest}>Manifest SHA-256 {artifact.sha256.slice(0, 12)}… · {(artifact.byteLength / 1024).toFixed(0)} KB · retrieved {artifact.retrievedAt}</Text> : null}
                </TouchableOpacity>
              );
            })}
          </View>
        ...
        <Text style={styles.version}>Privacy Lens 1.14.0 · Decision-calibration candidate</Text>
      </ScrollView>
...
function EvidenceStep({ icon, title, body, warning = false }: { icon: keyof typeof Ionicons.glyphMap; title: string; body: string; warning?: boolean }) {
  return <View style={styles.chainStep}><View style={[styles.chainIcon, warning && styles.chainIconWarning]}><Ionicons name={icon} size={19} color={warning ? T.colors.danger : T.colors.primary} /></View><View style={styles.chainCopy}><Text style={styles.chainTitle}>{title}</Text><Text style={[styles.chainBody, warning && styles.chainBodyWarning]}>{body}</Text></View></View>;
}
...
const styles = StyleSheet.create({
  ...
  attestationCard: { minHeight: 126, marginTop: 12, padding: 16, flexDirection: 'row', gap: 12, borderRadius: T.radius.medium, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: T.colors.mintStrong, ...T.shadow.card }, attestationCardWarning: { borderColor: '#E7B8AF', backgroundColor: '#FFF9F7' }, attestationIcon: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: T.colors.mint }, attestationIconWarning: { backgroundColor: T.colors.dangerSoft }, attestationCopy: { flex: 1 }, attestationTitle: { color: T.colors.primaryDark, fontSize: 14, lineHeight: 19, fontWeight: '900' }, attestationTitleWarning: { color: T.colors.danger }, attestationBody: { color: T.colors.inkSoft, fontSize: 12, lineHeight: 18, marginTop: 4 }, attestationBoundary: { color: T.colors.muted, fontSize: 10, lineHeight: 15, fontWeight: '700', marginTop: 7 },
  chainCard: { marginTop: 12, padding: 16, borderRadius: T.radius.medium, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: T.colors.line, ...T.shadow.card }, chainStep: { minHeight: 54, flexDirection: 'row', alignItems: 'center', gap: 11 }, chainIcon: { width: 38, height: 38, borderRadius: 13, alignItems: 'center', justifyContent: 'center', backgroundColor: T.colors.mint }, chainIconWarning: { backgroundColor: T.colors.dangerSoft }, chainCopy: { flex: 1 }, chainTitle: { color: T.colors.ink, fontSize: 12, lineHeight: 17, fontWeight: '900' }, chainBody: { color: T.colors.primaryDark, fontSize: 11, lineHeight: 16, marginTop: 2 }, chainBodyWarning: { color: T.colors.danger }, chainDivider: { height: 1, marginVertical: 7, marginLeft: 49, backgroundColor: T.colors.line }, chainBoundary: { color: T.colors.muted, fontSize: 10, lineHeight: 15, fontWeight: '700', marginTop: 12, paddingTop: 11, borderTopWidth: 1, borderTopColor: T.colors.line },
  ...
  sourceHeading: { marginTop: 29, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }, sourceHeadingCopy: { flex: 1 }, sourceCount: { minWidth: 34, height: 34, paddingHorizontal: 8, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: T.colors.mint }, sourceCountText: { color: T.colors.primaryDark, fontSize: 13, fontWeight: '900' }, sourceList: { gap: 9, marginTop: 13 }, sourceCard: { minHeight: 178, padding: 15, borderRadius: 17, borderWidth: 1, borderColor: T.colors.line, backgroundColor: '#FFFFFF', ...T.shadow.card }, sourceTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, sourceBadge: { paddingHorizontal: 8, paddingVertical: 5, borderRadius: T.radius.pill }, bindingBadge: { backgroundColor: T.colors.mint }, guidanceBadge: { backgroundColor: T.colors.infoSoft }, consultationBadge: { backgroundColor: T.colors.warningSoft }, sourceBadgeText: { color: T.colors.ink, fontSize: 9, lineHeight: 12, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 0.35 }, sourceTitle: { color: T.colors.ink, fontSize: 14, lineHeight: 19, fontWeight: '900', marginTop: 10 }, sourceVersion: { color: T.colors.inkSoft, fontSize: 11, lineHeight: 16, fontWeight: '700', marginTop: 4 }, sourceLifecycle: { alignSelf: 'flex-start', color: '#705826', fontSize: 10, lineHeight: 15, fontWeight: '900', marginTop: 7 }, sourceMeta: { color: T.colors.muted, fontSize: 11, lineHeight: 16, marginTop: 3 }, sourceReview: { color: T.colors.primaryDark, fontSize: 10, lineHeight: 15, fontWeight: '800', marginTop: 4 }, sourceDigest: { color: '#705826', fontSize: 9, lineHeight: 14, fontWeight: '800', marginTop: 5 }, sourceReviewDue: { color: T.colors.danger }, noSources: { minHeight: 74, marginTop: 13, padding: 15, flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 17, backgroundColor: T.colors.warningSoft, borderWidth: 1, borderColor: '#EAD9B2' }, noSourcesText: { flex: 1, color: '#705826', fontSize: 12, lineHeight: 18, fontWeight: '700' },
