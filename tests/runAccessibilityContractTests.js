...
assert(findingCard.includes('Witness receipts') && findingCard.includes('requiredWitnessCount') && findingCard.includes('witnessReceiptSetSha256'), 'Expanded findings must expose the witness threshold and canonical receipt-set identity.');
...
assert(findingCard.includes('Decision pause') && findingCard.includes('session-only check') && findingCard.includes('Closing this evidence card clears every answer.'), 'Decision support must disclose its ephemeral, non-collecting interaction boundary.');
assert(findingCard.includes('accessibilityRole="radio"') && findingCard.includes('accessibilityRole="checkbox"') && findingCard.includes('accessibilityLiveRegion="polite"'), 'The comprehension and context checks must expose selectable state and announced feedback.');
assert(findingCard.includes('A technical signal that needs context') && findingCard.includes('A GDPR violation') && findingCard.includes('Developer intent'), 'The comprehension check must test the signal-versus-verdict boundary explicitly.');
...
assert(settings.includes('trustStoreAssessment: trustStore') && settings.includes('Key governance & witnesses') && settings.includes('independent witness receipts verified') && settings.includes('root signature alone cannot make this trust store current') && settings.includes('do not bind an installed APK/AAB hash') && settings.includes('uninstall, app-data clearing, or device compromise') && settings.includes('public transparency log'), 'Settings must disclose witness status and the limits of release identity, artifact binding, and persisted rollback state.');
...
assert(settings.includes('Decision-pause answers') && settings.includes('cleared when the evidence card closes') && settings.includes('never stored or uploaded'), 'Settings must disclose that comprehension responses are ephemeral and non-collecting.');
