...
try {
  stdout = execFileSync('java', ['-Xmx4g', '-jar', jar, '-a', apk, '-p', platforms, '-s', definitions, '-o', xml, '-r', '-dt', '120', '-ct', '60', '-rt', '30', '-mt', '2'], { encoding: 'utf8', maxBuffer: 32 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] });
} catch (error) {
...
const receipt = {
  ...
  command: { reflectionEnabled: true, mainTimeoutSeconds: 120, callbackTimeoutSeconds: 60, resultTimeoutSeconds: 30, maxThreads: 2 },
  status: exitCode !== 0 ? 'FAILED_OR_TIMED_OUT' : fs.existsSync(xml) ? 'COMPLETED' : 'COMPLETED_NO_RESULT_ARTIFACT',
  exitCode,
