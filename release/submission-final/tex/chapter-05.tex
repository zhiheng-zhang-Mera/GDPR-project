...
Gradle completed \texttt{assembleRelease} and \texttt{bundleRelease} with target SDK 36 in the recorded build workflow. The 9 August receipt contains binary hashes for an earlier candidate and is retained as historical build-pipeline evidence; those hashes are not attributed to release \PrivacyLensVersion{}. The 8--24 research freeze identifies the mature source baseline and Android package metadata. The 8 September finalisation rebuilt the release APK and AAB from that source line and recorded their hashes; Play signing and store delivery remain external owner actions.
...
To test the engineering workflow beyond the project APK, a separate authorised corpus was constructed from the official F-Droid repository. This is an \emph{open-source app-store} corpus, not a closed-source commercial-store sample. The final hash-pinned static census contains \FDroidCensusN{} APKs; the device subset and its exclusion boundary are separately receipt-owned. The catalogue, hash boundary, and static census are recorded under \path{testing-report/}\allowbreak\path{fdroid-open-store-2026-08-22/}.
...
\subsubsection{Formal Properties, Mapping Review, and Accessibility Evidence}

The formal output model distinguishes admitted evidence, typed technical signals, review obligations, and prohibited legal conclusions. Nine registered safety properties have executable witnesses, and the curated mutation campaign detected all nine registered non-equivalent weakenings. Detection is executed rather than asserted: \texttt{npm run verify:mutation} copies the compilable sources into an isolated temporary tree, applies one declared source weakening per registered mutant, recompiles that tree, and requires the compiled compliance suite to fail on each one. Each transformation is anchored to an exact source string and the command aborts if an anchor has drifted, so a mutant cannot be silently skipped; a surviving mutant fails the command. The initial execution of this check found that four of the first candidate weakenings survived, two because they were equivalent to the original code and two because the cited suite did not witness them; the campaign was corrected to target the actual weakening in each case, and one property gained an explicitly anchored order-invariance assertion because its permutation loop had compared every permutation against a reference computed by the same engine. This observed 9/9 result is scoped to the curated mutant set; it is not a complete mutation score and does not prove absence of unmodelled defects.

The regulation-mapping review packet contains 13 atomic items with blinded reviewer fields and an explicit unsupported category. At thesis freeze, independent reviewer judgements and agreement statistics are not available. The review is therefore reported as \texttt{NOT\_RUN}; no agreement value, legal certification, or completed-review claim is inferred from preparation of the packet.
...
To strengthen the evidence beyond the repository corpus, \texttt{tests/runTemporalStressCampaign.ts} is executed from a fixed seed over the compiled delivered source. It runs \StressCorpusN{} randomised observation multisets over both registered packs against an independent count-and-window oracle, \StressRestartN{} random partition, export, restore and resume cases that require the restored temporal evidence receipt to equal uninterrupted evaluation, \StressMalformedN{} malformed-audit cases that require fail-closed rejection with a byte-for-byte unchanged exported temporal ledger, one valid 10,001-entry snapshot that must restore bounded to 10,000 entries, and forged, cross-pack, and expired snapshots that must be rejected. The campaign completes \StressAssertionsN{} assertions and matches every registered temporal rule \StressRuleMatchesN{} times in total. A non-vacuity guard fails the campaign if any compiled rule in any registered pack is never matched, so the corpus cannot silently stop covering a rule. \texttt{npm run reproduce:thesis-stress} executes the campaign twice and requires both runs to agree on every deterministic field before writing a receipt that records the host, the observed counts, and the per-rule match totals. These results establish reproducibility and the tested fail-closed invariants under the specified generated inputs; they are not a performance benchmark, proof of every possible input, legal validation, or field evidence.
